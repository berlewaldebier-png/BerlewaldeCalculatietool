...
- Build `Van verkoopprijs naar contributie`.
  ...
  - explicit category treatment for beer, giftsets, services and merchandise
...
5. Timeline and variance bridge. Start by wiring actual cumulative revenue from backend `timeline`; keep plan/reforecast phasing explicit until a real reforecast model exists. The first backend variance bridge is exact at total level: planned result plus contribution variance plus fixed-cost variance equals reforecast result. Price, volume and mix splits require frozen plan-mix detail and should not be guessed.
