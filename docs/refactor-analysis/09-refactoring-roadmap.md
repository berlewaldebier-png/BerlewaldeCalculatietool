...
flowchart TD
    ...
    R12D1 --> R12D1A["RF-012D1A Historical 14-step wizard view"]
    R12D1A --> R12D2["RF-012D2 Active cost overview"]
    R12D2 --> R12D3["RF-012D3 Cost variants and history"]
    R12D3 --> R12C2B["RF-012C2B Explicit Management Forecast"]
    R12C2B --> R12C3["RF-012C3 Exact LOT/non-LOT Actual costs"]
    R12C3 --> R12C4["RF-012C4 Sales strategy and advice prices"]
    R12C4 --> R14
...
- **RF-012C2B explicit Management Forecast:** Management and Administrator may append a new invoice-based monthly Forecast revision bound to the exact active generation, reconciliation run, immutable Plan row and Plan hash. Completed months remain exact Actual; the partial month may not reduce already invoiced cumulative inputs; future months are explicit user input. Contribution is always derived as revenue minus variable cost and may be negative. No backlog is inferred from unregistered orders. Previous revisions remain immutable audit history.
- **RF-012C3 Omzet en Marge actuals:** migrate exact LOT resolution to RF-011B separately. An exact LOT determines the cost version regardless of order/invoice date. Missing, unknown, near, ambiguous or multi-LOT input fails closed with a visible typed warning and never silently receives the planning cost. Non-LOT cost-bearing products use the explicit SKU/year planning anchor; `no_cost_required` remains a separate maintained policy.
...
- **RF-012C4A Verkoopstrategie:** switch SCREEN-017 only. The active generation supplies the complete stable-SKU scope, ownership group and planning cost. The exact `sales_pricing_records` target row supplies the mutable current `list` sell-in price; the generation candidate price remains an immutable activation snapshot. Save only explicitly changed SKU rows in one transaction, retain unknown payload fields, reject stale generation/run/manifest or record hashes and never replace/delete a complete price year. New quotations for the active generation resolve that same current target row; superseded generations remain on their immutable candidate snapshot. Keep the new-year wizard draft path unchanged.
- **RF-012C4B Adviesprijzen:** switch SCREEN-018 in a later branch only after RF-012C4A acceptance. Advice remains derived from the same current SKU sell-in authority plus the approved channel advice markup, VAT and existing five-cent rounding. Do not include RF-012C4B in the RF-012C4A branch.
- **RF-012C4 SKU-family SSOT prerequisite:** the active generation is authoritative for one financial row per stable `sku_id`, but shared bundle/service presentation membership must be modeled separately from SKU ownership before switching `/verkoopstrategie`. Preserve an explicit distinction between a SKU's owning Beer/product family and cross-family BOM/presentation references. A shared SKU may be referenced from multiple styles without being cloned into multiple financial records. Required SKUs without a sell-in price remain visible with a typed readiness reason instead of being silently filtered.
...
**RF-012C1 implementation note (2026-07-30):** brand-new quotations now read one quote-specific, read-only projection of the activated RF-013 generation. Each selectable stable SKU receives its candidate planning cost, approved SKU list/sell-in price and reserved target identities; every exclusion retains typed reason codes surfaced as actionable UI warnings. Quote payload schema 3 stores generation/run/year/manifest/validation identity. Reopened schema-3 quotes reload that exact active or superseded generation and never silently reprice; pre-RF-012C1 quotes are explicitly retained on their persisted legacy year/context. No schema, migration, backfill or historical quote rewrite is included. A read-only development check resolved the active 2026 generation with 79 candidate and 47 quote-ready SKUs; the 32 exclusions all lack a candidate SKU-specific target sell-in and remain visible by reason code. See `31-rf-012c1-quotation-active-context.md`.

**RF-012C2 implementation note (2026-07-30):** the current Break-even screen now resolves the active RF-013 commercial generation instead of the highest `productie` year. Plan is accepted only as the hash-verified immutable frozen Plan for the ready active generation/run; Forecast is separately projected as Actual completed periods plus the approved remaining period/SKU Plan, starts as an exact Plan copy and becomes exact final Actual after year close. Only an explicitly persisted Forecast revision bound to the same generation, run and Plan hash may override that projection. Historical explicit years remain on their compatibility reader. The consumer is read-only: no schema, migration, backfill, historical revaluation or persistent write is included. The development check resolved the active 2026 generation with 12 Plan periods and 60 SKU allocations; the authenticated UI showed populated Plan/Actual/Forecast and retained the existing missing-cost warning for RF-012C3. See `32-rf-012c2-break-even-plan-forecast.md`.
...
**RF-012C2B implementation note (2026-08-12):** Break-even now contains a responsive Management Forecast editor for the active commercial generation. Its initial values reuse the already-rendered Plan/Actual/Forecast read model; audit history loads only when requested. A save appends one revision to the new `commercial_forecast_revisions` authority, supersedes only the prior active pointer in the same transaction and revalidates generation/run/Plan/hash plus optimistic revision identity under locks. Four foreign keys use `ON DELETE RESTRICT`; no business row is migrated, backfilled, deleted or revalued. A strict read-only development check found the new table present with four restrictive foreign keys and zero revisions before manual acceptance. See `37-rf-012c2b-management-forecast.md`.

**RF-012C3 implementation note (2026-08-12):** Omzet en Marge now resolves new/recomputable Actual snapshots through the RF-013B planning-anchor and canonical exact-LOT authorities. Unknown, missing, near, ambiguous and multi-LOT input is visibly unresolved; non-LOT cost-bearing SKUs use their explicit transaction-year planning anchor. Canonical cost components travel with the existing snapshot payload. Normal sync protects all finalized snapshots and neither updates nor inserts rows before the active 2026 year; explicit targeted maintenance remains the only deliberate correction path. A strict read-only audit examined 3,080 invoice snapshots: 2,360 remain protected and 720 current-year provisional/missing rows are eligible for reconsideration. It also found 24 transaction/SKU keys with multiple LOTs, which now fail closed pending an approved allocation contract. No schema, migration, backfill or stored-data change was executed. See `38-rf-012c3-actual-costs.md`.

**Approved continuation order (2026-08-04):** after RF-012C2, continue with RF-012D1 (finalized yearset dossier), RF-012D1A (historical 14-step wizard view), RF-012D2 (active cost-price overview), RF-012D3 (cost variants/history), RF-012C2B (explicit Management Forecast), RF-012C3 (exact LOT/non-LOT Actual costs) and RF-012C4 (sales strategy/advice prices). RF-012C2B must not infer backlog from unregistered orders: orders enter this application only when invoiced, so Forecast revisions require an explicit Management input bound to generation/run/Plan hash.

**RF-012D1A/RF-012C4 SKU-family evidence amendment (2026-08-07):** the retained 2025→2026 engine batch has 15 distinct rows presented from Berlewalde Blond, but only seven are directly Blond-owned SKU records. The other eight are shared bundles/services; several occur under four to six Beer groups in the same batch. The finalized 2026 generation retains one financial row per stable SKU and has seven Blond-owned cost rows, six with a sell-in price. The still-legacy `/verkoopstrategie` reader derives rows from 2026 activations plus each active version's `cost_lines`; all seven Blond activations currently point to one version containing only the `Doos 6 * 75cl` cost line, so it cannot be treated as the same read authority. RF-012D2 and RF-012C4 must use the active generation and the explicit owner-versus-cross-reference projection; neither slice may infer ownership from the old repeated wizard presentation or delete/rewrite compatibility data.

**RF-012C4A implementation note (2026-08-14):** SCREEN-017 now reads one active-generation projection with every stable SKU exactly once. Beer-owned SKUs are grouped by canonical Beer; shared bundles, services and articles have their own owner groups and are not cloned through BOM/presentation references. The immutable candidate list price is retained as activation evidence, while the exact existing target record (or one unambiguous later SKU record) is the mutable current sell-in authority. Missing, non-positive, ambiguous and not-applicable prices remain visible with typed states. Administrator saves are atomic, optimistic and targeted: no year-wide replacement, delete, migration, generation mutation or historical rewrite occurs. New quotations for the active generation use the same live target row; superseded generations retain their snapshot. The strict read-only development audit returned 79 active SKU rows, 16 owner groups, 46 ready prices, 30 missing prices, one non-positive price, two not-applicable catalogue rows, four retained compatibility-only price rows, seven Blond-owned SKUs and zero duplicate stable SKU IDs. See `39-rf-012c4a-sales-strategy-active-context.md`.

**RF-012C4B implementation note (2026-08-14):** SCREEN-018 now reads the same active-generation SKU scope, owner grouping, planning cost and exact current SKU sell-in authority as SCREEN-017. The active generation fixes the year and retains immutable channel-markup snapshots; the exact live `advice_channel_pricing` row remains the mutable current advice policy. Existing VAT conversion, customer-margin calculation and five-cent inclusive downward rounding are reused without formula changes. Every active SKU remains visible with a typed ready, missing-cost, missing-sell-in, missing-VAT or not-applicable state. Missing VAT is no longer silently treated as 0% for a new calculation. Administrator saves are atomic, optimistic and limited to explicitly changed active-year channels; no broad dataset reconciliation, delete, migration, candidate mutation or historical rewrite occurs. The strict read-only development projection returned 79 SKU rows, 16 owner groups, four ready channel markups, 45 fully calculable advice rows, 31 missing-sell-in rows, one otherwise-ready missing-VAT row, two not-applicable rows and zero duplicate SKU IDs. Seven further missing VAT sources are already blocked earlier by missing sell-in. See `40-rf-012c4b-recommended-prices-active-context.md`.

### RF-012D — Cost overview, format defaults and immutable history dossier

- **Slice boundary:** RF-012D is implemented as separately reviewed branches/PRs. RF-012D1 introduces the finalized Jaarset dossier; RF-012D1A adds a historical read-only wizard projection to that dossier; RF-012D2 switches the current “Kostprijs beheren” overview; RF-012D3 adds cost variants/history. Approval of one identifier does not authorize a later identifier.
...
- **RF-012D1A historical wizard projection:** Jaarsetdossier gets an explicit `Jaarsetoverzicht` / `Wizardweergave` toggle. The wizard view reuses the same 14-step navigation contract as the editable new-year wizard but fixes source/target to the selected finalized generation (for Jaarset 2026: 2025→2026), exposes no save/activate/restore action and reads only retained evidence. Exact retained values, exact values derived from frozen sources, later reconstructed values and not-retained inputs must be visibly distinguished. The retained target-engine batch is deduplicated by stable SKU and must match the canonical active/superseded generation at six-decimal storage precision; ambiguity, conflicting duplicates or a material mismatch fails closed. This slice does not resurrect a draft, recalculate a cost price, infer missing inputs, restore values or write data. Restoration remains separately approved future work.
...
**RF-012D1A implementation note (2026-08-04):** the finalized Jaarset dossier now offers a read-only wizard view with the same shared 14-step structure as the active new-year wizard. For the retained 2025→2026 evidence, 103 historical presentation rows resolve to 74 stable SKU IDs; all 74 retained financial signatures match the canonical 2026 dossier at six-decimal precision. Five later canonical rows are shown only through their explicit exact-target-anchor or non-cost catalogue provenance. Plan, tariffs, fixed costs, packaging, per-SKU cost calculations, sell-in prices, channels and audit are labeled by source fidelity. The original inflation input itself was deleted with the draft, but 6.0% is derived exactly from 8/9 comparable fixed-cost rows and 25/25 packaging prices; source, inflated expectation and stored target remain separate. Four source/target production recipes, including Stout, are read from explicitly linked definitive cost versions; their stored target ingredient prices did not receive the 6.0% increase and that discrepancy is reported without rewriting history. Step 9 shows source and target total cost, and step 10 groups SKU prices per Beer/style family. The later-updated production/driver row remains explicitly reconstructed. The endpoint and UI are read-only and add no schema, migration, backfill, calculation activation, restoration or persisted-data change. See `34-rf-012d1a-historical-yearset-wizard.md`.

**RF-012D2 implementation note (2026-08-07):** the current `Kostprijs beheren` overview now reads the one active ready commercial generation through a `costs:view` endpoint and no longer derives current prices from legacy activation/version/UI fan-out. The projection groups once by canonical Beer owner and then concrete stable SKU; independent services, articles and bundles remain separate. `Doos 24 × 33cl` and keg receive the approved default order. Cost method and year-transition/recovery provenance are displayed separately. Valid cost, missing cost, not activated and not applicable are distinct states, and duplicate stable SKU IDs fail closed. A read-only shadow comparison reports scope divergence without changing either source. The 2026 development check returned 79 generation SKU IDs, 77 valid required costs, two `n.v.t.` rows and a known 79-versus-77 generation/legacy scope difference. The authenticated desktop and 390 × 844 checks confirmed 16 unique groups, correct Blond/Juweel/Weizen classification, local table scrolling and accessible disclosure controls. No schema, migration, backfill, activation, formula, cost value or persisted record changed. See `35-rf-012d2-active-cost-overview.md`.

**RF-012D3 implementation note (2026-08-07):** `Kostprijs beheren` now lazily exposes an explicitly read-only **Alle varianten / historie** dossier per stable SKU. The active commercial generation remains the only current planning SSOT; prior planning anchors and target-year backing rows are shown only as verified lineage, never as competing current selectors. The 2026 development projection contains 79 active SKU histories, 66 exact source-year-anchor bindings, eleven exact target-year anchor bindings (including the three recovered Juweel records), two `n.v.t.` rows, 170 retained cost-version rows, 93 additional variants and 59 exact canonical LOT links. Fifty-two LOT declarations exist only at cost-version level and 17 direct opening-stock LOT records lack canonical cost-version/SKU-row lineage; both are visibly unresolved and return no invented amount. Eight retained historical cost rows have a component-total mismatch and remain visible as evidence without repair. The GET route requires `costs:view`, starts read-only, rechecks the active generation binding and fails closed on duplicate identities or mismatched anchor lineage. No schema, migration, backfill, rebaseline, activation, cost formula, stored amount or historical record changed. See `36-rf-012d3-cost-history.md`.
...
**RF-013C3 development activation note (2026-07-30):** after merge, a fresh private `pg_dump` was verified readable before any write. The approved RF-013B manifest was applied and rerun idempotently; the exact C3 preview returned 79 SKUs, 77/77 required costs, 47/47 prices, four channels, a balanced EUR 220,000 Plan, 12 periods, 60 SKU allocations and zero blockers. Management approval and Administrator activation moved the active pointer to generation `5a152227-146c-5904-bb91-f8ef4d0b52ee` / run `636ff712-89a7-5a4c-87e8-d2a371cb0d8d`. Independent pre/post comparisons found no change in protected legacy tables, datasets, year totals or integrity fingerprints. The private backup and manifests remain under ignored `outputs/rf013p/`.
...
**RF-014B implementation note (2026-08-21):** the RF-011A `activeCommercialContextResolver.ts` and its exclusive synthetic contract are removed as one deprecated feature path. Repository and Git-history searches found no runtime caller since introduction: only the dedicated contract and pricing test configuration referenced the resolver. Live quotation, Break-even, sales-strategy and recommended-price consumers now use their accepted RF-012C backend-bound context paths; canonical year-transition still retains the shared planning selector. Executable guards require the removed path to stay absent and protect those current entrypoints. Shared types, planning utilities, historical quote support and the separately reviewable shadow module remain. No URL, API, formula, permission, schema or persisted data changes. See `42-rf-014b-unused-commercial-context-resolver.md`.
