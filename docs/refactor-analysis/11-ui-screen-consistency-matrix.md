...
| **SCREEN-018 — Recommended prices** `/adviesprijzen` | Audit found four unnamed number controls; local save/status handling differed from settings and pricing screens. RF-012B2 adds explicit names and shared status semantics. | Accessibility baseline **Observed**; RF-012B2 correction **implemented**; financial risk **medium/high**. **P0/P1**. | Preserve the RF-012B2 boundary and contracts; do not change price calculations, rounding, defaults or save granularity before RF-012C/RF-013. | F2 completed after pricing fixtures; source migration remains deferred. |
