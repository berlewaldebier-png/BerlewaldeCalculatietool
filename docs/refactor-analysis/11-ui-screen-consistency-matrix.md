...
| Mobile screenshots show the standard sidebar expanding into a long full-width navigation block before feature content. | **Observed.** | Shell/mobile navigation needs an explicit product decision and RF-012 route-by-route rollout; it must not be silently changed by a primitive extraction. |
...
|---|---|---|
| **F1 — Focus and semantic tokens** | Visible focus, semantic foreground/background/border states, reduced-motion hook, intentional token exceptions. | RF-009, one CSS/component family at a time. |
| **F2 — Accessible name and async status** | Programmatic field names, row/column context, `role=status`/`alert`, loading/busy semantics. | RF-009, starting with a low-risk shared component. |
...
| **F4 — Dialog behavior** | Accessible title, initial focus, tab containment, Escape/outside policy, focus restoration, cancel/no-write. | RF-009 one non-destructive dialog family at a time; destructive/financial dialogs require workflow characterization. |
| **F5 — Table taxonomy** | Separate contracts for read-only analytics, editable datasets, admin tables, wizard tables, and CPQ lines. | RF-009 defines shared semantics/cells; RF-012 handles page-specific responsive composition. |
| **F6 — Shell and responsive hierarchy** | PageShell variants, mobile navigation, title/breadcrumb placement, content widths and feature breakpoints. | RF-012 after product/design confirmation; not an RF-009 primitive sweep. |
...
| **F8 — Terminology and product content** | Authoritative Dutch vocabulary, placeholder labeling, version/notification/deployment copy. | Human product/operations decision; mostly not primitive work. |
...
| **SCREEN-002 — Dashboard** `/` | Own sidebar/content frame rather than PageShell; first semantic heading in the audit was chart-oriented rather than a stable page H1. One serious contrast issue and a search hydration warning occurred. | Layout/accessibility **Observed**; intentional analytics exception **Unknown**. **P1/P3**. | Formalize an analytics-shell variant instead of forcing the dashboard into the standard card. Add a stable page-name contract and shared status/focus semantics. Investigate hydration warning independently. | F1/F2; F6 requires confirmation. |
...
| **SCREEN-029 — Company settings** `/instellingen/bedrijf` | Low-complexity settings form; one serious contrast node in representative scan. Save status is local, although RF-008 centralized its API boundary. | Accessibility/visual **Observed**. **P1 / Reference**. | Use as the first low-risk status-message reference: preserve fields, copy, payload and save behavior; trace contrast to the owning token. | F1/F2. |
| **SCREEN-031 — Notifications** `/instellingen/meldingen` | Static “Binnenkort” cards coexist with a hard-coded global notification badge, implying live functionality. | Content/workflow mismatch **Observed**; intended product state **Unknown**. **P3**. | Clearly label demo/coming-soon behavior or hide the live-looking badge after product decision. Do not build notifications in RF-009. | F8. |
...
| **SCREEN-020 — Quote builder** `/offerte-samenstellen` | Distinct CPQ family; five serious contrast nodes, an unnamed customer search input, `.cpq-input/.cpq-select/.cpq-textarea` remove outlines without a replacement focus rule, and several local dialogs lack shared focus lifecycle. Mobile composition remains recognizable but headings wrap tightly. | Accessibility **Observed**; CPQ visual distinction likely intentional **Inferred**. **P0/P1, high workflow risk**. | Keep CPQ styling separate. Name the customer search and introduce a CPQ-specific visible focus token after screenshots. Handle option/volume dialogs in their own characterized slice. | F1/F2, then F4/F5; not the first RF-009 family. |
...
| **SCREEN-022 — Scenario analysis** `/scenario-analyse` | Own filter/input controls and browser persistence; analogous analytics interactions are not shared with Break-even. | Interaction/state inconsistency **Observed**. **P2**. | Document local-persistence and override semantics before sharing filters/status. Retain scenario-specific behavior. | F2/F5; RF-012. |
...
| **SCREEN-033 — User administration** `/beheer/users` | English “Users” conflicts with Dutch navigation. Edit/deactivate dialogs are labelled but lack shared focus lifecycle. Mobile title is tightly wrapped/partly obstructed by the development badge, and the create-user grid becomes narrow. | Terminology/accessibility/responsive **Observed**. **P1**. | Obtain terminology decision. Consider the edit-user dialog as a later non-destructive dialog pilot after save/cancel tests; handle mobile form composition in RF-012. | F4/F6/F8. |
...
|---|---|---|
| **SCREEN-G01 — Header** | Hard-coded notification count/version; search/breadcrumbs disappear at different widths; account-menu behavior is local. | Separate product content from interaction behavior. Header menu/search keyboard work belongs in F3; notification/version decisions in F8. |
...
| **SCREEN-G03 — PageShell/navigation** | At mobile width, the full navigation list precedes primary content and makes every standard screen extremely long. | Product must choose drawer/collapse/compact navigation behavior. Implement in RF-012, not as a global RF-009 CSS edit. |
...
| **SCREEN-S02/S03 — Error boundaries** | Retry exists, but raw error message/digest presentation differs from feature errors and may disclose internals. | Use RF-008 error metadata to separate user message from diagnostic details; preserve retry/login actions. |
...
- **Changes:** row + column accessible names; save/error live-status semantics; explicit busy state relationships where missing.
...
- **Regression tests:** axe label result, accessible-name assertions, keyboard Tab sequence, save success/error, duplicate-submit protection, screenshots at desktop and 390px.
...
1. **RF-009C:** low-risk shared status primitive on SCREEN-004/029.
...
## Human decisions required before visible consolidation

1. Is the dashboard intended to remain a distinct analytics shell?
2. Should mobile navigation become a drawer/collapsible menu, or is the current full navigation list intentional?
3. Should the CPQ/quote visual family remain deliberately denser and visually distinct? The recommendation is **yes**.
4. Which Dutch terms are authoritative: `Users` or `Gebruikers`; `Kostprijs beheer`, `Kostprijs beheren`, or `Kostprijsbeheer`; `Scenario analyse` or `Scenario analyseren`?
5. Should custom dialogs close on outside click? Should Escape always cancel? Which element receives initial focus?
6. Which semantic color combinations are approved for muted text, badges and warning states after contrast measurement?
7. Should placeholders such as notifications remain visible as “Binnenkort”, or should unavailable affordances be hidden?
8. Which table categories must support mobile editing versus read-only horizontal scrolling?
