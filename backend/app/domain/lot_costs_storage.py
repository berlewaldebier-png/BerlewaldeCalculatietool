...
def list_internal_lot_summary(*, year: int = 0, limit: int = 5000) -> list[dict[str, Any]]:
    """Return internal LOTs grouped by beer style from cost versions only.

    This is intentionally independent from Douano sales LOT allocations. It is
    a light read model for the data-quality screen: which LOT numbers do our
    cost versions/purchase invoices currently carry per style?
    """
    from app.domain import cost_versions_storage
    ...
    where = "WHERE v.status = 'definitief' AND COALESCE(NULLIF(l.lot_number, ''), '') <> ''"
    params: list[Any] = []
    if year_value > 0:
        where += " AND v.jaar = %s"
    ...
    with postgres_storage.connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"""
                SELECT
                    COALESCE(NULLIF(v.bier_id, ''), NULLIF(v.payload->'basisgegevens'->>'bier_id', ''), '') AS style_id,
                    COALESCE(
                        NULLIF(v.payload->'basisgegevens'->>'biernaam', ''),
                        NULLIF(v.payload->'basisgegevens'->>'naam', ''),
                        NULLIF(v.payload->>'biernaam', ''),
                        NULLIF(v.bier_id, ''),
                        'Onbekende stijl'
                    ) AS style_name,
                    l.lot_number,
                ...
                {where}
                ORDER BY style_name, l.lot_number, v.versie_nummer
                LIMIT %s
                """,
                tuple(params),
            )
    ...
    for row in rows:
        style_id, style_name, lot_number, version_id, version_year, version_number, source_type, source_ref, source_date = row
        style_id_text = _text(style_id)
        style_name_text = _text(style_name) or style_id_text or "Onbekende stijl"
        group_key = style_id_text or style_name_text.lower()
        ...
        version_num = int(version_number or 0)
        ...
        year_num = int(version_year or 0)
        if year_num > 0 and year_num not in lot_item["years"]:
            lot_item["years"].append(year_num)
        source_label = _text(source_ref) or _text(source_type)
    ...
    for group in groups.values():
        ...
        lots.sort(key=lambda item: _text(item.get("lot_number")).upper())
        out.append(
            {
    ...
    out.sort(key=lambda item: _text(item.get("style_name")).lower())
    return out
